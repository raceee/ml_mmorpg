...
class Raid:
    ...
    def make_fight(self):
        ...
        while self.player_party_total_health > 0:
            self.player_party_total_health -= self.boss.boss_attack() # block needs to be added here
            boss_damage_tape.append(self.player_party_total_health)
        player_damage_tape = []
        while self.boss.boss_health > 0:
            self.boss.boss_health -= sum([player.player_attack() for player in self.player_list]) # block needs to be added here
